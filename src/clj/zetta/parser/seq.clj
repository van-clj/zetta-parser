(ns zetta.parser.seq
  ...
  (:require ^{:cljs [cljs.core :as core]}
            ...
            [zetta.parser.combinators :refer :all])
  ^:clj (:import [zetta.parser.core Parser]))
  ...
  (Parser.
   (fn [input0 more0 err-fn ok-fn]
     (if (>= (count input0) n)
       ...
       ((zetta-macro/>> demand-input (ensure n)) input0 more0 err-fn ok-fn)))))
...
(defn skip-while
  ...
  [pred]
  (let [skip-while-loop (zetta-macro/do
             [input0 get
